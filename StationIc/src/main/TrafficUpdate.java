...
public class TrafficUpdate {
	...
	public static void setCollectionName(String collectionName) {
		TrafficUpdate.collectionName = collectionName;
	}
			...
			if (i < array.size() - 1) {
				if (bus.equals(array.get(i + 1).getString("busselfId"))) {
					...
					if (ord == 0) {
						...
						traffic = QueryTraffic.getTraffic(db2, array.get(i), array.get(i), array.get(i + 1), inter);
						UpdateTraffic.update(db2, collectionName, id, traffic);
					} else{
						traffic = QueryTraffic.getTraffic(db2, array.get(i), array.get(i - 1), array.get(i + 1), inter);
						UpdateTraffic.update(db2, collectionName, id, traffic);
					}
					...
					isSameBus =0;
					if (ord == 0) {
						traffic = QueryTraffic.getTraffic(db2, array.get(i), array.get(i), array.get(i), inter);
						UpdateTraffic.update(db2, collectionName, id, traffic);
					}else{
						traffic = QueryTraffic.getTraffic(db2, array.get(i), array.get(i - 1), array.get(i ), inter);
						UpdateTraffic.update(db2, collectionName, id, traffic);
					}
					System.out.println(" "+traffic);
				}

...
public class stationPrediction {
	...
	public static double[][][] getPre_station(String startTime, String endTime){
			...
			if(i<segment.size()){
				...
				for(int j=1;j<= station;j++){
					if(j==1&&j<=loc-2){
						...
						tensor[i][j-1][0]=(array2.get(loca)<stateSpace?array2.get(loca):stateSpace-1+array3.get(loca)<=stateSpace?array3.get(loca):stateSpace-1)/2;
	...
	public static void saveToFile_station(String startTime, String endTime){
		double [][][] tensor = getPre_station(startTime, endTime);
		File file_pre = new File(".\\pre_station\\prediction.txt");
		...
		for(int i =0; i<lineNum;i++){
			for(int j=0;j<station;j++){
				File file = new File(".\\pre_station\\init_"+i+"_"+j+".txt");

...
public class GeneralPreSS {
	private GeneralTransitionSS generalTrans ;
	...
	public GeneralPreSS(int segmentId,String stationId, String startTime, String endTime, int mod){
		this.segmentId = segmentId;
		...
		generalTrans = new GeneralTransitionSS();
	...
	public void prediction(){
		...
		result = Matrix.multip_vector(generalTrans.getTransiton(segmentId,stationId, startTime, endTime,mod), state, generalTrans.getStateSpace());
	}
	/**
	 * Ò»²½×ªÒÆÔ¤²â
	 * @param res
	...
	public int prediction(double[] res, int sta){
		...
		state[sta]=1.0;
		res = Matrix.multip_vector(generalTrans.getTransiton(segmentId,stationId, startTime, endTime,mod), state, generalTrans.getStateSpace());
	...
	public int predictionByInitState(double[] res, int sta){
		...
		res = Matrix.multip_vector(generalTrans.getTransiton(segmentId,stationId, startTime, endTime,mod), res, generalTrans.getStateSpace());
		return DealVector.getMax(res, generalTrans.getStateSpace());
	...
	public double   acc_top2(String time1, String time2){
		...
		generalTrans.getTransiton(segmentId,stationId, startTime, endTime, mod);
		array = sequence.findBydayProcess(segmentId,stationId, time1, time2, mod);
		//array = sequence.findProcess(stationId, time1, time2, mod);
		System.out.println(array);
		//double[] res = generalTrans.getInitState();
		for( int i =0 ;i < array.size()-1; i++){
	...
	public static void main(String []args){
		...
		GeneralPreSS generalPre  = new GeneralPreSS(1,"12111300000000045252", startTime, endTime, 60*60);
